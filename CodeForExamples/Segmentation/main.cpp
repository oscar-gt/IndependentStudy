...
// Code borrowed from OpenCV by Example text
...
int main( int argc, const char** argv )
{
  ...
  medianBlur(img, img_noise, 3); // originally had value of 3 *********************
  blur(img, img_box_smooth, Size(3,3)); // original Size(3, 3) *****************
  ...
  medianBlur(light_pattern, light_pattern, 3); // Originally had value of 3 **************************

	Mat img_noise2;
	// Improve contrast *******************************************************************
	//equalizeHist( img_noise, img_noise2 );
	//equalizeHist( img_noise2, img_noise );
	// Improve contrast *******************************************************************
	
  //Apply the light pattern
  Mat img_no_light;
  img_noise.copyTo(img_no_light);	// Changed to image_noise2 ************************
  if(method_light!=2){
  	img_no_light= removeLight(img_noise, light_pattern, method_light);	// Originally has method_light as last param ********************
  }															  // ALSO CHANGED TO img_noise2 in prev line!!!!!!!!!!!!!			
  
  
  // Binarize image for segment
  Mat img_thr;
  if(method_light!=2){
	  cout << "30, 255, THRESH_BINARY USED" << endl;
  	threshold(img_no_light, img_thr, 120, 255, THRESH_BINARY);	// Originally 30, 255
  }else{
	  cout << "140, 255, THRESH_BINARY_INV USED" << endl;
  	threshold(img_no_light, img_thr, 140, 255, THRESH_BINARY_INV);
  }
  
  // Show images
  miw->addImage("Input", img);
  miw->addImage("Input without noise", img_noise);						// Changed to img_noise2 !!!!! ****************8
